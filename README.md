...
Softmax is defined as follows: for vector $z \in \mathbb{R}^K$ with elements $z_{i}, \all i \in \mathbb{Z}^{+} \leq K$  $$\text{softmax}(z)_{i} = \frac{\exp{z_{i}}}{\sigma_{j = 1}^{K} \exp{z_{j}}}$$
