...
class UnlimiformerOriginal(Generic[ModelType]):
    """
    Functions that are used but not implemented here:

    - create_key_value
    - process_key_value
    - process_query
    - attention_layer_to_capture
    - attention_op_to_run
    - attention_layer_to_run
    - self_attention
    - cross_attention
    - window_size
    - create_decoder_layer_args
    ...
    def inject_hooks(self, model):
        ...
        for i, decoder_layer in enumerate(decoder_layers_to_run):
            ...
            self.cross_attention(decoder_layer).forward = self.create_cross_attn_pre_forward_hook(
    ...
    def inject_training_hooks(self, model):
        """
        hook all the model's functions with custom functions made for Unlimiformer,
        First, get all modified decoder layers, then change all the original
        forward functions to self attention and cross attention pre forward functions
        """
    ...
    def reset_memory(self, input_ids, attention_mask):
        """
        Reset memory initializes self.datastore as DatastoreBatch and initializes
        all the variables in the class including prompt_input_ids, input_ids,
        prompt_keys, prompt_values, prev_tokens, last_beam_idx, cur_layer_key_value_placeholder,
        is_input_encoding_pass, generated_input_ids, prompt_attention_mask
        It then loops through the sequence through window indices and obtains
        keys from model for each chunk and adds it to datastore if use_datastore
        If use_datastore, it is trained after the loop

        """
        if self.use_datastore:
            self.datastore = DatastoreBatch(dim=self.model.config.hidden_size, batch_size=input_ids.shape[0],
                                            flat_index=self.flat_index, gpu_index=self.gpu_index)
            self.embeddings = []
            torch.cuda.empty_cache()
        ...
        for context_start_ind, context_end_ind, update_start_ind, update_end_ind in window_indices:
            ...
            hidden_states = self.model(chunk, attention_mask=chunk_attention_mask, labels=dummy_labels,
                                       return_dict=True)
    ...
    def chunked_encode_input(self, input_ids, attention_mask):
        """
        Given sequences of very long text, we first get window indices in
        ...
        the arxivrecsep120000ppl23 output is (batch, source_len, dim) of encoder outputs
        """
        long_inputs_encoded = []
        long_inputs_mask = []
        window_indices = self.window_indices(input_ids.shape[-1])
        ...
        for context_start_ind, context_end_ind, update_start_ind, update_end_ind in window_indices:
            ...
            # needs modification for decoder-only model
            output = self.model.base_model.encoder(chunk, attention_mask=chunk_attention_mask, return_dict=True,
                                                   output_hidden_states=True)
            encoder_last_hidden_state = output.last_hidden_state  # (batch, time, dim)

            # list of (batch, head, chunked_time, dim)
            encoder_last_hidden_state = encoder_last_hidden_state[:,
                                        update_start_ind:update_end_ind]  # (batch, chunked_time, dim)
            chunk_attention_mask = chunk_attention_mask[:, update_start_ind:update_end_ind]  # (batch, chunked_time)
    ...
    def pre_forward_hook(self, input_ids=None, attention_mask=None, labels=None, **kwargs):
        """
        A function that acts as an original forward function but preprocesses the input if
        self.is_input_encoding_pass is false, in that case it calls chunked_encode_input
        if self.model.training is true else is concatenates the input ids
    ...
    def create_cross_attn_pre_forward_hook(self, original_cross_attn_forward_func, decoder_layer, i):
        """
        Creates a modified cross attention forward function that converts past key value
        from a tuple to a list to perform assignments later on
        If self.model.training, Reshape the input hidden states and attention mask before
        calling original cross attn forward, then reshape attention output back to its
        original shape to be returned
        """
        def attention_pre_forward_hook(hidden_states, attention_mask=None, *args, **kwargs):
            ...
            if self.model.training:
                ...
                attn_output, attn_weights_reshaped, past_key_value = original_cross_attn_forward_func(
                    hidden_states=hidden_states, attention_mask=attention_mask, *args, **kwargs)
                attn_output = attn_output.reshape(batch_size, tgt_len, dim)
                return attn_output, attn_weights_reshaped, past_key_value
            else:
                return original_cross_attn_forward_func(hidden_states=hidden_states, attention_mask=attention_mask,
                                                        *args, **kwargs)
    ...
    def attention_forward_hook(self, module, input, output):
        ...
        with torch.no_grad():
            ...
            if self.use_datastore:
                ...
                k_proj = k_proj.view(1, self.num_heads, query.shape[-1],
                                     k_proj.shape[0])  # (1, num_heads, attn_dim, embed_dim)
                ...
                datastore_query = datastore_query.view(
                    (self.datastore.batch_size, -1, datastore_query.shape[2]))  # (batch, beam * num_heads, embed_dim)
                # then search
                if self.reconstruct_embeddings:
                    # embeddings: (batch, beam * head, actual_model_window_size, dim)
                    top_search_key_scores, top_search_key_indices, embeddings = self.datastore.search_and_reconstruct(
                        datastore_query, k=self.actual_model_window_size)
                else:
                    top_search_key_scores, top_search_key_indices = self.datastore.search(datastore_query,
                                                                                          k=self.actual_model_window_size)
                    ...
                    embeddings = torch.take_along_dim(input=self.embeddings.unsqueeze(1),
                                                      indices=top_search_key_indices.unsqueeze(-1).to(
                                                          self.embeddings.device), dim=-2)
                    embeddings = embeddings.to(self.device)
                # (batch, beam, head, actual_model_window_size)
                top_search_key_scores = top_search_key_scores.reshape(
                    (self.datastore.batch_size, -1, self.num_heads, self.actual_model_window_size))
                top_search_key_indices = top_search_key_indices.reshape(
                    (self.datastore.batch_size, -1, self.num_heads, self.actual_model_window_size))
                # embeddings: (batch, beam, head, actual_model_window_size, dim)
                embeddings = embeddings.reshape((self.datastore.batch_size, -1, self.num_heads,
                                                 self.actual_model_window_size, embeddings.shape[-1]))
            ...
            if (not self.use_datastore) or self.test_datastore:
                ...
                attn_weights = torch.matmul(this_layer_prompt_keys.unsqueeze(1)[:, :, self.head_nums],
                                            query.unsqueeze(-1)).squeeze(-1)
                ...
                top_key_scores, top_key_indices = torch.topk(attn_weights, k=topk, dim=-1,
                                                             sorted=True)  # (batch, beam, head, trunc_source)
                if self.save_heatmap:
                    # heatrow: (beam, heads, source_len)
                    heatrow = torch.zeros(
                        [top_key_indices.shape[1], top_key_indices.shape[2], this_layer_prompt_keys.shape[-2]],
                        dtype=torch.float)
        ...
        if self.use_datastore:
            ...
            k_weight = k_proj_layer.weight.view(1, 1, self.num_heads, embed_dim // self.num_heads, embed_dim).transpose(
                -2, -1)  # (1, 1, heads, embed_dim, attn_dim)
            k_bias = k_proj_layer.bias.view(1, self.num_heads, embed_dim // self.num_heads).unsqueeze(-2)
            v_weight = v_proj_layer.weight.view(1, 1, self.num_heads, embed_dim // self.num_heads, embed_dim).transpose(
                -2, -1)  # (1, heads, embed_dim, attn_dim)
            ...
            new_values = torch.matmul(embeddings, v_weight) + v_bias.unsqueeze(
                0)  # (beam, head, encoder_len, embed_dim)
        else:
            # this_layer_prompt_keys:   (batch,       head, source_len, dim)
            ...
            new_values = torch.take_along_dim(this_layer_prompt_values.unsqueeze(1),
                                              indices=top_key_indices.unsqueeze(-1),
                                              dim=-2)  # (batch, head, trunc_source, attn_dim)

        if self.test_datastore:
            correct_keys = torch.take_along_dim(this_layer_prompt_keys.unsqueeze(1),
                                                indices=top_key_indices.unsqueeze(-1),
                                                dim=-2)  # (batch, head, trunc_source, attn_dim)
            correct_values = torch.take_along_dim(this_layer_prompt_values.unsqueeze(1),
                                                  indices=top_key_indices.unsqueeze(-1),
                                                  dim=-2)  # (batch, head, trunc_source, attn_dim)
    ...
    def reorder_cache_hook(self, past, beam_idx):
        """
        A function that updates the class variables including last_beam_idx, generated_input_ids,
        and prev_tokens before passing it into original reorder cache func
        Calling this function is the same as calling the original function except modifying
        class variables
        """
...
class ActivationCapturer(nn.Module):
    """
    A wrapper around a layer that captures either input or
    output of a layer according to capture_input.
    It then stores it in self.captured to be used.
